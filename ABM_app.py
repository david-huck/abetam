...
start_year = st.select_slider(
    ...
    simplified_heating_systems.reset_index()["REF_DATE"].unique(),
)
...
heating_techs_df["annuity_factor"] = discount_rate / (
    1 - (1 + discount_rate) ** -heating_techs_df["lifetime"]
)
heating_techs_df["annuity"] = (
    heating_techs_df["annuity_factor"] * heating_techs_df["specific_cost"]
)
...
heating_techs_df["vom_cost[EUR/a]"] = (
    demand / heating_techs_df["efficiency"] * heating_techs_df["specific_fuel_cost"]
)

heating_techs_df["emissions[kg_CO2/a]"] = (
    demand / heating_techs_df["efficiency"] * heating_techs_df["specific_fuel_emission"]
)

heating_techs_df["total_cost[EUR/a]"] = heating_techs_df[
    ["invest_cost[EUR/a]", "fom_cost[EUR/a]", "vom_cost[EUR/a]"]
].sum(axis=1)

heating_techs_df.loc[:, ["emissions[kg_CO2/a]_norm", "total_cost[EUR/a]_norm"]] = (
    heating_techs_df[["emissions[kg_CO2/a]", "total_cost[EUR/a]"]]
    .apply(normalize)
    .values
)
